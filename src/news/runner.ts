...
const START_DATE_STRING = moment().format('YYYYMMDD')
...
async function run () {
  const startDate = moment(START_DATE_STRING)
  ...
  console.time('time')
  do {
    const { from, to } = generateDateRange(cursorDate)
    const { successList, failedList } = await runner(from, to)
    success.push(...successList)
    failed.push(...failedList)
    cursorDate.subtract(DATE_RANGE_AMOUNT, DATE_RANGE_UNIT)
    console.log(`success: ${success.length}, failed: ${failed.length}`)
  } while (startDate.diff(cursorDate, END_CONDITION_UNIT_OF_TIME) < END_CONDITION_AMOUNT)
  console.log(`complete scraping with query ${QUERY}`)
  console.timeEnd('time')

  writeJson(`${QUERY}_success.data.json`, success)
  writeJson(`${QUERY}_failed.data.json`, failed)
}
